  ...
  .mutation(async ({ input }) => {
    ...
    const response = await invokeLLM({
      messages: [
        {
          role: "system",
          content: `你是企業客服 AI。所有對客輸出必須使用自然、專業、簡潔的繁體中文。只能根據提供的知識與規則回答，不可捏造 Excel 不存在的資訊。若信心低於 0.80、規則衝突或需要個案處理，必須誠實說明並提出下一步。資訊不足時只能補問一個最必要的問題，不能一次問多題。不得索取完整信用卡號、CVV、密碼。請將回答控制在清楚的 2 至 5 段。`,
        },
        {
          role: "user",
          content: JSON.stringify({ message, history, context: { ...context, knowledge: knowledgeText } }),
        },
      ],
      response_format: {
        type: "json_schema",
